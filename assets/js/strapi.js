...
const apiURLhero = "strapi-ge-production.up.railway.app/api/hero";
...
async function fetchHeroContent() {
  try {
    ...
    if (heroData.Background && heroData.Background.url) {
      const backgroundUrl = `strapi-ge-production.up.railway.app${heroData.Background.url}`;
      const baseUrl = "strapi-ge-production.up.railway.app"; // Cambia esto si usas otra URL base para Strapi
...
async function loadContent() {
  try {
    const response = await fetch("strapi-ge-production.up.railway.app/api/hero?populate=*");
    if (!response.ok) {
    ...
    if (data && data.data) {
      ...
      if (heroData.Background && heroData.Background.length > 0) {
        ...
        const fullBackgroundUrl = backgroundUrl.startsWith("http")
          ? backgroundUrl
          : `strapi-ge-production.up.railway.app${backgroundUrl}`;
...
async function fetchHeroBackgrounds() {
  try {
    const response = await fetch(
      "strapi-ge-production.up.railway.app/api/hero?populate=Background"
    );
...
function renderBackgroundImages(backgroundImages) {
  ...
  backgroundImages.forEach((image) => {
    ...
    imgElement.src = `strapi-ge-production.up.railway.app${image.url}`;
...
function updateImagePreview() {
  ...
  if (previewElement && selectedImageUrl) {
    previewElement.style.backgroundImage = `url(strapi-ge-production.up.railway.app${selectedImageUrl})`;
...
async function saveBackground() {
  ...
  try {
    ...
    const response = await fetch("strapi-ge-production.up.railway.app/api/hero", {
      method: "PUT",
      headers: {
...
function updateHeroBackground() {
  ...
  if (heroElement && selectedImageUrl) {
    heroElement.style.backgroundImage = `url(strapi-ge-production.up.railway.app${selectedImageUrl})`;
...
// URL de la API de Strapi
const apiURLabout = "strapi-ge-production.up.railway.app/api/about";
...
async function loadAboutContent() {
  ...
  try {
    const response = await fetch("strapi-ge-production.up.railway.app/api/about");
    if (!response.ok) {
      throw new Error(
...
async function fetchAboutGeidypic() {
  try {
    const response = await fetch(
      "strapi-ge-production.up.railway.app/api/about?populate=Geidypic"
    );
    ...
    if (geidypicImages && geidypicImages.length > 0) {
      ...
      const firstImageUrl = `strapi-ge-production.up.railway.app${geidypicImages[0].url}`;
...
function updateGeidypicPreview() {
  ...
  if (previewElement) {
    previewElement.style.backgroundImage = `url(strapi-ge-production.up.railway.app${geidypicImageUrl})`;
...
// <!-- Funciones para trabajar com el HERO desde la API de Strapi-->
// URL de la API de Strapi
const apiURLstat = "strapi-ge-production.up.railway.app/api/stat";
...
async function loadStatContent() {
  try {
    const response = await fetch("strapi-ge-production.up.railway.app/api/stat");
    if (!response.ok) {
      throw new Error("Error al obtener el contenido: " + response.status);
    }
    const data = await response.json();
...
// URL de la API de Strapi
const apiURLoffice = "strapi-ge-production.up.railway.app/api/office";
...
async function loadOfficeContent() {
  ...
  try {
    const response = await fetch("strapi-ge-production.up.railway.app/api/office");
    if (!response.ok) {
      throw new Error(
...
async function login() {
  ...
  try {
    ...
    const response = await fetch("strapi-ge-production.up.railway.app/api/auth/local", {
      method: "POST",
